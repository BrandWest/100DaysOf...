...
sed -i -E "s/\| ([0-9]{4}-[0-9]{2}-[0-9]{2}) \| ([0-9]+) \| [a-f0-9]{7} - (.*)/| $TODAY | $COMMITS_TODAY | ${LAST_COMMIT_HASH:0:7} - $LAST_COMMIT_MESSAGE |/" ".github/workflows/test.md" 

# # git add "$CURRENT_DIR/README.md"
# # git commit -m "Update commit table with today's date and last commit details [skip ci]"
# # git push
